...
ticketRouter.post('/orders', requireAuth, async (req: AuthRequest, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    for (const item of body.items) {
      ...
      await tx.ticketCategory.update({ where: { id: item.categoryId }, data: { soldCount: { increment: item.quantity } } });
    }
...
ticketRouter.get('/my', requireAuth, async (req: AuthRequest, res) => {
  const tickets = await prisma.ticket.findMany({ where: { userId: req.auth!.userId } });
  res.json(tickets);
});

ticketRouter.post('/issue/:orderId', async (req, res) => {
